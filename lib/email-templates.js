...
// Three roles, three different messages. Written as a lookup rather than a
// boolean, because the boolean version silently told a new managing editor
// they had been "added as a reviewer" and sent them to the reviewer page --
// anything that is not exactly 'editor' fell through to the reviewer wording.
const ROLE_COPY = {
  managing_editor: {
    page: 'editor.html',
    subject: 'You have been given managing editor access — Aaranya Scholarly',
    heading: 'Managing editor access granted',
    body:
      'You now have editorial access across every journal in the portfolio, ' +
      'including the ability to grant and remove other editors. Submissions ' +
      'and reviewer comments are confidential — please treat them accordingly.',
    cta: 'Open editorial dashboard',
  },
  editor: {
    page: 'editor.html',
    subject: 'You have been given editor access — Aaranya Scholarly',
    heading: 'Editor access granted',
    body:
      'You can now screen submissions, invite reviewers, and record editorial ' +
      'decisions from the editorial dashboard, for the journals assigned to you.',
    cta: 'Open editorial dashboard',
  },
  reviewer: {
    page: 'reviewer.html',
    subject: 'You have been added as a reviewer — Aaranya Scholarly',
    heading: 'You are now a reviewer',
    body:
      'Editors may now invite you to review manuscripts. You will receive an ' +
      'email each time you are invited, and can accept or decline each one ' +
      'individually.',
    cta: 'Open reviewer dashboard',
  },
};

function roleGranted({ name, role }) {
  const copy = ROLE_COPY[role] || ROLE_COPY.reviewer;
  const link = url(copy.page);
...
You have been given the ${label} role for Aaranya Scholarly journals.
...
Aaranya Scholarly LLP`,
    html: layout({
      ...
      bodyHtml:
        p(`Dear ${esc(name)},`) +
        p(`You have been given the <b>${esc(label)}</b> role for Aaranya Scholarly journals.`) +
        p(copy.body),
      ctaLabel: copy.cta,
      ctaUrl: link,
    }),
