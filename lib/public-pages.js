...
const { licenceFor } = require('./publication');
...
function articlePage({ article, issue, fullTextHtml }) {
  ...
  // The licence is stored as a key ("cc-by-4.0"). Readers need the name and,
  // more importantly, the deed itself -- "Open Access" on the masthead means
  // nothing unless the article states the terms and links to them.
  const licence = licenceFor(article.license);
  ...
  const metaRows = [
    ...
    ['Licence', licence ? licence.label : ''],
  ]
    .filter((r) => r[1])
    .map((r) => {
      let value = esc(r[1]);
      if (r[0] === 'DOI') {
        value = `<a href="${attr(r[1])}" style="color:var(--teal);">${esc(r[1])}</a>`;
      } else if (r[0] === 'Licence' && licence && licence.url) {
        // rel=license is what machines read to establish the terms.
        value = `<a href="${attr(licence.url)}" rel="license" style="color:var(--teal);">${esc(
          r[1]
        )}</a>`;
      }
      return `<div><div class="k">${esc(r[0])}</div><div class="v">${value}</div></div>`;
    })
    .join('\n');
